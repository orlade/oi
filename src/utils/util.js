import {exec as _exec, env, pwd} from 'shelljs'
...
export function exec(command, options = {}, callback = null) {
  let message = `Executing ${command.cyan} in ${pwd().magenta}`;
  if (options) {
  ...
  if (options.detached) {
    ...
    result = spawn(main, args, {detached: true, stdio: ['ignore', 'ignore', 'ignore']});
    result.unref();
  } else {
    result = _exec(command, options, callback);
  }
 ...
 *
 * @param {string} name The name of the object running the task.
 ...
 */
export function reportResult(name, task, result, time, {info = false} = {}) {
  const timeStr = `(${time.asSeconds()} secs)`;
  if (determineSuccess(result)) {
    log[info ? 'info' : 'debug'](`${name} ${task} completed successfully ${timeStr}`.green);
  } else {
    let message = `${name} ${task} failed`;
    if (result) {
